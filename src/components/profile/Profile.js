...
import {
  ...
  updatingProfile,
  fetchingProfile,
} from '../../redux/features/profile.feature';
...
const Profile = () => {
  ...
  const updateState = useSelector((state) => state.profile.updating);
  const fetchingState = useSelector((state) => state.profile.fetching);
            ...
            {profile && Object.keys(profile).length > 0 && (
              <div className="profile" id="profile">
                <h4>
                  ...
                  <span>
                    <a
                      onClick={() => dispatch(updatingProfile())}
                      onKeyDown={() => dispatch(updatingProfile())}
                      role="link"
                      tabIndex={0}
                      className="inline-flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-white focus:z-10 focus:ring-2"
                ...
                <ul>
                  ...
                  <li>
                    <b>birth Date: </b>
                    {profile.birthdate ? new Date(profile.birthdate).toLocaleString() : ''}
                  </li>
                  <li>
                    <b>Currency: </b>
                    {profile.currency}
          ...
          <div className="profile-form">
            <form className="px-8 pt-6 pb-8 mb-4 bg-white rounded" onSubmit={handleSubmit}>
              {error && (
                <Alert
                  message={
                    typeof error.payload === 'string' ? error.payload : 'Unable to update profile'
                  }
                  heading="Error"
                  variant="error"
                />
